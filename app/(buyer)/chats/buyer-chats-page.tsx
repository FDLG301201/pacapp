  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          store:stores!store_id(id, name, logo_url, slug)
        `)
        .eq('buyer_id', userId)
        .order('last_message_at', { ascending: false })

      if (error) {
        console.error('Conversations load error:', error.message, error.code)
      }
      if (!error && data) {
        setConversations(data as ConversationWithStore[])
        // Auto-select: prefer URL param, then first conversation
        if (data.length > 0 && !selectedId) {
          setSelectedId(convParam ?? (data as ConversationWithStore[])[0].id)
        }
      }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          // Skip messages sent by me — already shown via optimistic update
